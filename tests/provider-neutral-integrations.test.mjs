...
test("transactional mail routes Gmail and SES without fallback", () => {
  ...
  for (const path of [
    "src/pages/api/astropages/generated-site/human-design.ts",
    ...
    assert.match(read(path), /sendTransactionalEmail/);
    assert.doesNotMatch(read(path), /sendSesTransactionalEmail/);
  }
});
