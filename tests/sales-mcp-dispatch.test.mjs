...
test("routes authenticated Users Data MCP calls through the shared endpoint", async () => {
  const response = await maybeHandleAnalyticsMcpToolCall(new Request("https://example.com/_emdash/api/mcp", {
    method: "POST",
  ...
  assert.equal(response.status, 200);
  const payload = await response.json();
  assert.equal(payload.result.structuredContent.contract, "users-data.v1");
  assert.equal(payload.result.structuredContent.method, "users_schema");
});
