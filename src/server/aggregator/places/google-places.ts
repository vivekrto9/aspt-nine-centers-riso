...
const getTimezoneOffset = (timezone: string, instant: Date) => {
  try {
    ...
    if (offsetName) return offsetName === "GMT" ? "UTC+00:00" : offsetName.replace("GMT", "UTC");
  } catch {
    // Fall through for runtimes without longOffset support.
  }
  ...
  const localAsUtc = Date.UTC(
    value("year"), value("month") - 1, value("day"), value("hour") % 24,
...
export const mapCoordinatesToTimezone = ({ lat, lon, date, time }: {
  lat: number;
  lon: number;
  date: string | null;
  time: string | null;
  ...
  return { timezone, offset: getTimezoneOffset(timezone, getTimezoneInstant(date, time)) };
};
...
export const mapGooglePlacePrediction = (prediction: Record<string, unknown>) => {
  const formatting = prediction.structured_formatting && typeof prediction.structured_formatting === "object"
...
export const mapGooglePlaceDetails = ({ result, date, time }: {
  ...
  const geometry = result.geometry && typeof result.geometry === "object"
    ...
    : {};
  const location = geometry.location && typeof geometry.location === "object"
    ? geometry.location as Record<string, unknown>
    : {};
  const lat = Number(location.lat);
  const lon = Number(location.lng);
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) throw new Error("Place details did not include coordinates.");
